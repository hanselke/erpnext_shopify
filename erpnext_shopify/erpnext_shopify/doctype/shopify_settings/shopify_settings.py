...
def make_item(warehouse, item):
    existing_erp_item = frappe.db.sql("""select item_name, description, item_group, attributes from tabItem where shopify_id=%(shopify_id)s""", {"shopify_id": item.get("id")}, as_dict=1)
