...
def create_customer(customer):
    ...
    if erp_customer:
        ...
        cust_name = customer.get("first_name") if (customer.get("first_name").strip().startswith('-') or customer.get("first_name").strip().startswith('00000') == -1) else customer.get("first_name").strip() + u'-' + str(uuid.uuid4())
        try:
